...
protocol ReloadDataBuying {
    func Reload (X:Int)
}

class AddBuyingViewController: UIViewController {
    ...
    var addsalesview: AddBuyingView! {
        guard isViewLoaded else { return nil }
        return (view as! AddBuyingView)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "AddBuyingProducts" {
            if let vc = segue.destination as? AddBuyingProductsViewController {
                if F == 1 {
                    vc.Flag = "0"
                    vc.delegate = self
                }
        ...
        if (x == 1) {
            ...
            if Flag == true {
                ...
                if (self.addsalesview.CustomerNameTextField.text == "" || self.addsalesview.NoteNumberTextField.text == "") {
                    ...
                    Firestore.firestore().collection("Byuedresete").document(id.uuidString).setData(ReseteData) {
                        error in
...
extension AddBuyingViewController: UITableViewDataSource {
...
extension AddBuyingViewController: UITableViewDelegate {
...
extension AddBuyingViewController: UITextFieldDelegate{
...
extension AddBuyingViewController: YCActionSheetDatePickerDelegate {
...
extension AddBuyingViewController: AddedProdcutsBuying {
